...
for (let i = 0; i < inputComment.length; i++) {
    inputComment[i].addEventListener('keyup', () => {
        if (inputComment[i].value) {
            postingButton[i].disabled = false;
        } else postingButton[i].disabled = true;
    })
}

//comment input
for (let i = 0; i < postingButton.length; i++) {
    postingButton[i].addEventListener('click', () => {
