...
for (let k = 0; k < inputComment.length; k++) {
  inputComment[k].addEventListener("keyup", (e) =>
    e.keyCode === 13 ? showComments(inputComment[k].value, k) : undefined
  );
}

for (let j = 0; j < inputComment.length; j++) {
  commentBtn[j].addEventListener("click", () =>
    showComments(inputComment[j].value, j)
  );
}
