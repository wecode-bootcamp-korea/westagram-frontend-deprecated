...
const profileMenuBtn = document.querySelector(".main-navbar__profile");
const profileMenuBox = document.querySelector(".profileMenuBox");
const IDArr = [
  "wecode_bootcamp",
  "wecode_founder",
  "wecode_Korea",
...
const makeResultElements = (searchResult) => {
  for (let i in searchResult) {
    ...
    <img
      ...
      src ="${searchResult[i][1]}"
      />
    <div class="profile-container__text">
      <span class="profile-container__name">${searchResult[i][0]}</span>
      <span class="profile-container__subtext">${searchResult[i][0]}</span>
    </div>
  </div>`;
